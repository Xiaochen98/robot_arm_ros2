...
class DofbotTrack(Node):
    def __init__(self):
        ...
        # self.sub_depth = self.create_subscription(Image,'/camera/depth/image_raw',self.GetDepthInfo,1 )
    ...
    def Clamping(self,cx,cy,cz):
        ...
        request.tar_z = pose_T[2]  + (math.sqrt(request.tar_y**2+request.tar_x**2)-0.181)*0.26
        request.kin_name = "ik"
        request.roll = self.CurEndPos[3] 
        print("calcutelate_request: ",request)
